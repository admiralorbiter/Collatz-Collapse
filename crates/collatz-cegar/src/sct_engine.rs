...
mod tests {
    ...
    fn test_composition_associativity() {
        let mut g1 = SizeChangeGraph::new("Q1", "Q2", vec![1, 1, 2]);
        g1.add_relation("v2_L1", "v2_L1", RelationValue::Strict);
        g1.add_relation("v2_L2", "v2_L1", RelationValue::Weak);

        let mut g2 = SizeChangeGraph::new("Q2", "Q1", vec![1, 2, 2]);
        g2.add_relation("v2_L1", "v2_L2", RelationValue::Weak);
        g2.add_relation("v2_L2", "v2_L2", RelationValue::Strict);

        let mut g3 = SizeChangeGraph::new("Q1", "Q2", vec![1, 1, 2]);
        g3.add_relation("v2_L1", "v2_L1", RelationValue::Weak);

        let g12 = g1.compose(&g2).unwrap();
        let g12_3 = g12.compose(&g3).unwrap();

        let g23 = g2.compose(&g3).unwrap();
        let g1_23 = g1.compose(&g23).unwrap();

        assert_eq!(g12_3.matrix, g1_23.matrix);
    }

    #[test]
    fn test_saturation_closure_fixed_point() {
        let mut g12 = SizeChangeGraph::new("Q1", "Q2", vec![1, 1, 2]);
        g12.add_relation("v2_L1", "v2_L1", RelationValue::Strict);
        g12.add_relation("v2_L2", "v2_L1", RelationValue::Weak);

        let mut g21 = SizeChangeGraph::new("Q2", "Q1", vec![1, 2, 2]);
        g21.add_relation("v2_L1", "v2_L1", RelationValue::Weak);
        g21.add_relation("v2_L2", "v2_L2", RelationValue::Strict);

        let generators = vec![g12, g21];
        let closure = SctEngine::compute_saturation_closure(&generators, 100).unwrap();

        assert!(closure.len() >= 4);
        assert!(SctEngine::check_sct_termination(&closure).is_ok());
    }
}
